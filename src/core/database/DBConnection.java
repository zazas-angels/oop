...
public class DBConnection implements core.database.Connection {
    ...
    private synchronized AdminInterface getAdmin(String email, String password, CategoryTree categoryTree, boolean superAmin) {
        ...
        try {
            if (results.next()) {
                if (superAmin)
                    admin = new SuperAdministrator(results.getInt("ID"), email, password, this, categoryTree);
                else
     ...
     * @param parentCategoryId parent category's id, if(parentCategoryId is negative) it
